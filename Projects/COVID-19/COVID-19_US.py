...
dataUS = data[(data['Country/Region'] == 'US')].reset_index(drop=True)
dataUS_op = dataUS.groupby(['ObservationDate', 'Country/Region'])[
...
# Подтверждённые
figure.add_trace(go.Scatter(x=dataUS_op['ObservationDate'],
                            y=dataUS_op['Confirmed'],
                            mode='lines+text',
...
figure.add_annotation(
    x='03/19/2020',
    y=dataUS_op['Confirmed'].max(),
    text='COVID Lockdown in US',
...
figure.add_shape(
    ...
    dict(
        type='line',
        x0='04/19/2020',
        y0=dataUS_op['Confirmed'].max()-30000,
        x1='04/19/2020',
        line=dict(
            color='green',
            width=5
...
figure.update_layout(title='Evolution of Confirmed, Active cases in US', template='plotly_white')
